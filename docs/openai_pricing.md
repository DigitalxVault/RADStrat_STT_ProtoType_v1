...
Notes:
- For faster processing of API requests, try the priority processing service tier.
- For lower prices with higher latency, try the flex processing tier.
- Large numbers of API requests which are not time-sensitive can use the Batch API for additional savings.
- Reasoning tokens are billed as output tokens (even if not visible via the API).
...
Notes:
- Tokens used for model grading in reinforcement fine-tuning are billed at the model’s per-token rate.
- Inference discounts may be available if you enable data sharing when creating the fine-tune job.
...
| Web search preview (reasoning models, incl. gpt-5, o-series) | $10.00 / 1k calls + search content tokens billed at model rates |
| Web search preview (non-reasoning models) | $25.00 / 1k calls + search content tokens are free |

Notes:
- Web search costs include (1) tool calls and (2) search content tokens (billed at the chosen model’s input token rate unless otherwise specified).
...
`omni-moderation` models are free of charge.
